...
fun test_ln_wad_accuracy() {
    let cases: vector<LnCase> = vector[
        ...
        LnCase {
            p: (SCALE / 2) as u128,
            expected_mag: 693_147_180_559_945_309u256,
            expected_neg: true,
            tolerance: 500_000_000_000_000_000,
        }, // ln(0.5) = -ln2
        LnCase {
            p: 100_000_000_000_000_000u128,
            expected_mag: 2_302_585_092_994_045_000u256,
            expected_neg: true,
            tolerance: 2_000_000_000_000_000_000,
        }, // ln(0.1)
    ];
    let mut i = 0;
...
fun test_sqrt_wad_accuracy() {
    ...
    let diff_half = if (quarter > expected_half) { quarter - expected_half } else {
        expected_half - quarter
    };
...
fun test_ppf_tail_accuracy_points() {
    let cases: vector<TailCase> = vector[
        TailCase {
            p: 100000000,
            expected_mag: 6_361_340_889_697_422_000u256,
            expected_neg: true,
            tolerance: 5_000_000_000_000_000,
        }, // p = 1e-10
        TailCase {
            p: 1_000_000_000_000,
            expected_mag: 4_753_424_308_822_899_000u256,
            expected_neg: true,
            tolerance: 5_000_000_000_000_000,
        }, // p = 1e-6
        TailCase {
            p: 1_000_000_000_000_000,
            expected_mag: 3_090_232_306_167_813_000u256,
            expected_neg: true,
            tolerance: 3_000_000_000_000_000,
        }, // p = 1e-3
    ];
    let mut i = 0;
    while (i < std::vector::length(&cases)) {
...
fun test_sampler_invariants() {
    let seeds = vector[
        9334618346840842905u64,
        10424100709723369767u64,
        9443182695686696458u64,
        17933673198744876582u64,
        11342960890024317363u64,
        10482980246973705466u64,
        5290281280504745347u64,
        10228930864023155047u64,
        8624286824275906926u64,
        11253583944051605563u64,
        17163634629999896190u64,
        4535784743480959579u64,
    ];
...
fun test_sampler_normal_mean_shift() {
    ...
    let avg = signed_wad::div_wad(
        &sum,
        &signed_wad::from_wad((std::vector::length(&seeds) as u256) * SCALE),
    );
...
fun test_ppf_edges_and_symmetry() {
    ...
    let diff_low = if (mag_low > expected_edge) { mag_low - expected_edge } else {
        expected_edge - mag_low
    };
    let diff_high = if (mag_high > expected_edge) { mag_high - expected_edge } else {
        expected_edge - mag_high
    };
...
fun test_sampler_ppf_moments() {
    ...
    let diff_var = if (second_moment > SCALE) { second_moment - SCALE } else {
        SCALE - second_moment
    };
