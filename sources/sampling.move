...
/// ```
public struct SamplerGuard has drop, store {
...
fun sample_standard_normal_clt_with_gen(gen: &mut random::RandomGenerator): (u256, bool) {
...
public(package) fun sample_normal_from_u64(raw: u64, mean: u256, std_dev: u256): SignedWad {
    assert!(std_dev > 0, EInvalidStdDev);
...
/// ```
#[allow(lint(public_random))]
public fun sample_z(r: &random::Random, ctx: &mut sui::tx_context::TxContext): SignedWad {
    let z = sample_standard_normal_ppf_internal(r, ctx);
